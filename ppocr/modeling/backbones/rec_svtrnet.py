        ...
        if mode == 'pope':
            if sub_num == 2:
                self.proj = nn.Sequential(
                    ConvBNLayer(
                        in_channels=in_channels,
                        out_channels=embed_dim // 2,  #out_channels=32
                        kernel_size=3,
                        stride=2,
                        padding=1,
                        act=nn.GELU,
                        bias_attr=None),
                    ConvBNLayer(
                        in_channels=embed_dim // 2,   #in_channels=32
                        out_channels=embed_dim,       #out_channels=64
                        kernel_size=3,
                        stride=2,
                        padding=1,
                        act=nn.GELU,
...
class SVTRNet(nn.Layer):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            ...
            sub_num=sub_num)
        num_patches = self.patch_embed.num_patches  # num_patches = 200
        self.HW = [img_size[0] // (2**sub_num), img_size[1] // (2**sub_num)]   # HW = [8,25]
