...
class BaseRecLabelDecode(object):
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        for batch_idx in range(batch_size):
            ...
            #     if conf_list[i] is not None and conf_list[i] < 0.6:
            #         index_to_delete.append(i)
                    # numpy.delete(conf_list, i, axis=0)
                    # char_list[i] = ' ' * len(char_list[i])
            #conf_list = np.delete(conf_list, index_to_delete)
            # for j in index_to_delete:
            #     char_list[j] = ' ' * len(char_list[j])
            if len(conf_list) == 0:
                conf_list = [0]
            text = ''.join(char_list)
